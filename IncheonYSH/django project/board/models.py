...
class Article(models.Model):
    ...
    username = models.ForeignKey(User, on_delete=models.DO_NOTHING, db_column='username', related_name='username_article')
    title = models.CharField(max_length=30)
    maintext = models.TextField()
    time = models.DateTimeField(auto_now_add=True, auto_now=False)
    viewcount = models.PositiveIntegerField(default=0)
    like = models.ManyToManyField(User, related_name='like_article')
    dislike = models.ManyToManyField(User, related_name='dislike_article')
    def __str__(self):
